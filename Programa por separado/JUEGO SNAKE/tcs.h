...
int apple[3];
char score[3];// score [0] es el puntaje actual, score [1] es el récord más alto, score [2] controla la velocidad límite
char tail[3]; 
...
int Eat(char snake[2])   
{
    ...
    {
        apple[0] = apple[1] = apple[2] = 0;//Cuando appple [2] es 0, el programa reasignará la ubicación de la manzana.
        gotoxy(N+44,10);
        color(13);//color
...
{
    if (apple[2]) 						//Si el número de manzanas no es 0, dibuja manzanas
	{
        gotoxy(apple[1] * 2, apple[0]); //Dibuja la posición de la manzana (abscisas, ordenadas)
        color(12);//color
        printf("?");//manzana?
        color(11);
    }
    gotoxy(tail[1] * 2, tail[0]);// Dibuja la posición de la cola de la serpiente (abscisas, ordenadas)
    if (tail[2]) //Si el número de colas de serpiente no es 0, después de comer la manzana, la cuadrícula detrás de la cola de serpiente se agregará al cuerpo de la serpiente.
    {  
		color(14); 
        printf("?");//cuerpo ?
        color(11);
...
void init(char plate[N+2][N+2], char ***snake_x, int *len) 
{
    ...
    *len = 3;							
    ...
    for (i = 1; i <= N; ++i)				//El rango móvil es 1
        for (j = 1; j <= N; ++j) 
            plate[i][j] = 1;
...
void Manual()	//introduccion						
{
    gotoxy(N+30,2);
    color(10);
    printf("Presione W S A D para mover la dirección");
    gotoxy(N+30,4);//Ubicación de la información
...
int File_in()     //Tomar puntuación grabada
{
	FILE *fp;   
	if((fp = fopen("01.txt","a+")) == NULL)	
   	...
   	if((score[1] = fgetc(fp)) != EOF);		//leer archivo 
   		...
   		score[1] = 0;//La puntuación más alta registrada en el archivo al principio es 0
   	return 0;
}
...
int File_out()    //guardar 
{
    ...
    // Cuando la puntuación obtenida es superior al récord
	...
	gotoxy(10,10);
    color(12);
    puts("Felicitaciones por romper el récord"); 
    gotoxy(0,N+2);
    return 0;
}
